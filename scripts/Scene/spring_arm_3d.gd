...
## 控制x轴上的旋转速度
@export var x_speed: float = 25
## 控制y轴上的旋转速度
@export var y_speed: float = 25
## 最小角度(摄像机与地面的夹角)
@export_range(0, 90) var x_min_limit_angle : float = 0
## 最大角度(摄像机与地面的夹角)
@export_range(0, 90) var x_max_limit_angle : float = 90
## 默认相机视距(弹簧臂长度)
...
## 视距滚动的速度
@export var distance_speed: float = 50
## 视角转动 - 是否需要阻尼
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseMotion:
		...
		y = y + -event.relative.x * y_speed * 0.0002
		# 根据视角角度边界，来限制x的值
